...
# Extract classes, observed sizes and the data_dir from the config.
read -r DATA_DIR CLASSES OBSERVED_SIZES < <("$PY" - "$CONFIG" <<'PY'
import sys, yaml
cfg = yaml.safe_load(open(sys.argv[1]))
print(cfg["data_dir"], " ".join(cfg["class_names"]), " ".join(str(s) for s in cfg["observed_sizes"]))
PY
)
